...
public class FenModifierClient extends Stage {
	...
	public FenModifierClient()
	{
		...
		bnOk.setOnAction(e -> 
		{
			...
			else if (!tel.getText().matches("(0|\\+33)[1-9]( *[0-9]{2}){4}") && !tel.getText().matches("[0-9]{3}-[0-9]{3}-[0-9]{3}-[0-9]{3}"))
			{
			...
			{
				Principale.getClient(numeroClient).setClient(civilite, nom.getText(), prenom.getText(), adresse.getText(), adresse2.getText(), codePostal.getText(), ville.getText(), tel.getText(), mail.getText());
				Principale.modifierClient(Principale.getClient(numeroClient));
				Alert alerteCreation = new Alert(AlertType.INFORMATION, "Le client a bien été modifié", ButtonType.OK);
				alerteCreation.setTitle("Confirmation de modification");
				nom.clear();
				prenom.clear();
				adresse.clear();
				adresse2.clear();
				...
				this.close();
				//Recharger la liste des Clients après la modification
				Principale.rechercherClient();
				Principale.actualiserDonnees();
	...
	public void init(Client e)
	{
		if (e.getCivilite().equals("Monsieur"))
		{
			monsieur.setSelected(true);
		}
		else if (e.getCivilite().equals("Madame"))
		{
		...
		adresse.setText(e.getAdresse1());
		adresse2.setText(e.getAdresse2());
		tel.setText(e.getNumero_de_telephone());
		mail.setText(e.getEmail());
		ville.setText(e.getVille());
		codePostal.setText(e.getCode_postale());
		numeroClient = e.getNumero();
	}
